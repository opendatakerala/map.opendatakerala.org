const initializeLeafletPanelLayers = (L) => {    
    L.Control.PanelLayers = L.Control.Layers.extend({
        ...
        addOverlayGroup: function (group) {
            if (group.group && group.layers) {
                isCollapsed = group.collapsed || false;
                for (n in group.layers)
                    this._addLayer(group.layers[n], true, group.group, isCollapsed);
                this._update();
                // this.onAdd(this._map);
                return this;
            } else {
                throw ("Didn't get a group")
            }
        },

        check: function(layer) {
            layer._input.checked = true;
            this._onInputClick();
            this.fire('panel:selected', layer);
        },
...
module.exports = initializeLeafletPanelLayers;